# ABDULWAHED MANSOUR

**Security Researcher · Systems Engineer · Protocol Architect**

Mejramvägen 37, 145 73 Norsborg, Stockholm, Sweden
+46 76 930 8145 | abdulwahed.sweden@gmail.com
[GitHub](https://github.com/abdulwahed-sweden) | [LinkedIn](https://www.linkedin.com/in/abdulwahed-sweden/) | [Portfolio](https://abdulwahed-mansour.dev)

---

March 18, 2026

Dear Hiring Manager,

I am writing to express my strong interest in contributing to your organization as a Security Researcher, Systems Engineer, or Protocol Architect. With extensive hands-on experience building secure, high-performance systems across backend infrastructure, smart contract security, and blockchain protocols, I am confident in my ability to bring significant value to your team.

My work spans security research, Rust systems engineering, Python backend architecture, and DeFi protocol design. I discovered and formalized the ADS (Asymmetric Deficit Socialization) vulnerability class — a ~$98.6M phantom yield exposure affecting major lending protocols including Aave V4, Morpho Blue, and Curve crvUSD. I am an active auditor on Sherlock and Immunefi with confirmed findings in production protocols.

On the engineering side, I have built and maintain 21+ repositories totaling 208,000+ lines of production code with 5,700+ tests. My recent work includes high-performance async services in Rust (Tokio/Axum), Python backends (FastAPI/Django), smart contract security tooling (Foundry/Solidity), and AI-integrated systems using the Model Context Protocol (MCP). Several of these projects are hosted in private repositories due to their sensitive nature and are available for review upon request.

Jag ser fram emot att bidra till ert team och växa tillsammans med er organisation. Tack för att ni tar er tid att granska min ansökan!

I would welcome the chance to discuss how my experience aligns with your needs.

Best regards,
**Abdulwahed Mansour**
Security Researcher & Systems Engineer

---

## Attached Profiles

| # | Profile |
|---|---------|
| 01 | Senior Python Backend Architect |
| 02 | Senior Rust Systems Engineer |
| 03 | Smart Contract Security Auditor |
| 04 | Blockchain Security Researcher |
| 05 | DeFi Protocol Engineer — Islamic Finance |
| 06 | Full-Stack Web Developer |
| 07 | DevOps & Cloud Infrastructure Engineer |
| 08 | Security Engineer & Pentester |
| 09 | Blockchain Forensics Specialist |
| 10 | AI-Integrated Backend Engineer |

**Portfolio:** [abdulwahed-mansour.dev](https://abdulwahed-mansour.dev)
